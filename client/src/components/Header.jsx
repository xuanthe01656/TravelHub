...
const Header = ({ isLogged, welcomeMessage, handleLogout }) => {
  ...
  // State riêng cho việc mở rộng mục cài đặt trên Mobile
  const [mobileSettingOpen, setMobileSettingOpen] = useState(null); // 'lang' | 'curr' | null
          ...
          {isLogged ? (
            <div className="flex items-center gap-3 pl-2">
              <NavLink to="/profile" className="flex items-center gap-2 hover:opacity-80 transition-opacity">
                <div className="w-9 h-9 bg-blue-100 rounded-full flex items-center justify-center text-blue-600 font-bold border-2 border-white shadow-sm ring-1 ring-blue-100">
                  {welcomeMessage?.charAt(0).toUpperCase() || <FaUserCog />}
                </div>
                <div className="flex flex-col">
                  {/* Changed "Member" to t('auth.member') */}
                  <span className="text-[10px] text-slate-400 font-bold leading-none uppercase">{t('auth.member')}</span>
                  <span className="text-sm font-black text-slate-700 max-w-[100px] truncate">{welcomeMessage}</span>
                </div>
              </NavLink>
              <button onClick={handleLogout} className="p-2 rounded-full text-slate-400 hover:bg-red-50 hover:text-red-500 transition-all">
                <FaSignOutAlt />
              </button>
            </div>
          ) : (
            <button
              ...
              className="bg-blue-600 hover:bg-blue-700 text-white px-6 py-2.5 rounded-2xl font-black text-sm shadow-lg shadow-blue-100 transition-all active:scale-95 hover:shadow-blue-200"
            >
              {t('auth.login')}
            </button>
          )}
          ...
          <div className="flex-1 overflow-y-auto">
            ...
            {isLogged ? (
              <div className="p-6 bg-white mb-4 flex items-center gap-4">
                ...
                <div>
                  <p className="text-[10px] font-bold text-blue-600 uppercase tracking-tighter">{t('auth.member')}</p>
